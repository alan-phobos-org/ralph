...
def format_tool_result(result_content: str, is_error: bool, tool_summary: str) -> str:
    ...
    if has_error:
        error_msg = truncate_text(result_content, 80)
    ...
    if tool_summary.startswith('⚡ Bash'):
        ...
        if output_preview:
            ...
            return f"  ✓ Exit {exit_code}: {preview}"
        else:
...
Examples:
  ...
  %(prog)s "Build feature" --system-prompt "You are an expert Python developer"
    ...
    parser.add_argument('--outer-prompt', type=str, default=None, help='Path to outer prompt template file (default: prompts/outer-prompt-default.md)')
